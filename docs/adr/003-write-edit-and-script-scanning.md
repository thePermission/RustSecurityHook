...
**2. Script content scanning:** After the command string itself passes the blacklist and forbid checks, `rsh` splits the command on shell separators (`;`, `&&`, `||`, `|`, newline) and inspects each segment for script invocations. Recognised forms: `bash/sh/zsh/… <path>`, `python/python3/perl/ruby/node/nodejs <path>`, `source <path>`, `. <path>`, and direct execution (`./script.sh`, `/abs/path`, `name.sh`, `name.bash`). Segment tokenization now uses `shell-words` with a lightweight fallback parser for malformed shell input.

Before reading the referenced file, common home-directory forms (`~`, `~/...`, `$HOME/...`, `${HOME}/...`) are expanded. General shell expansion, glob expansion, command substitution, and arbitrary variable expansion are not performed.
...
- `bash -c 'echo hi'` — the inline string after `-c` is not a file path, so the segment remains a direct command and is covered by the command-string scan.
- `bash ~/deploy.sh` is scanned when `$HOME/deploy.sh` exists. A literal `~/deploy.sh` path is still tried as a fallback if home expansion does not resolve.
